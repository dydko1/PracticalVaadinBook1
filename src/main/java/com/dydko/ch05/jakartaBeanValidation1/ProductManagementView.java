...
@Route("ch05/product-management6")
public class ProductManagementView extends Composite<Component> {
    ...
    protected Component initContent() {
        ...
        return new VerticalLayout(
                new Button("Nówka", VaadinIcon.PLUS.create(),
                        e -> showProductForm(new Product())),
                productsLayout
        );
    }
    ...
    private void updateList() {
        ...
        products.stream().map(product -> new Details(
                product.getName() + (product.isAvailable() ? "yes" : "not available"),
